...
class AttachModel extends Model
{
    ...
    // 게시판 이미지 업로드
    public function uploadBoardImage($user_file)
    {
        $file_model = new FileModel();

        $limit_size = 10; // 2메가 바이트 업로드 제한 사이즈 메가바이트 단위로 입력
        $width = 0; // 가로 해상도 160, 가로 해상도로 0을 입력하면 세로를 기준으로 리사이징을 한다
        $height = 0; // 세로 해상도에 따라 조정, 세로 해상도로 0을 입력하면 가로를 기준으로 리사이징을 한다
        ...
        $proc_result["file_html"] = "<img src=\"/attach/view/".$file_id."\">";

        return $proc_result;
    }

    // 게시판 파일 업로드
    public function uploadBoardFile($user_file)
    {
        $file_model = new FileModel();

        $result = true;
        $message = "파일 업로드가 잘 끝났습니다.";
        $file_id = "0";
        $file_name_org = "error";

        $mime_type = $user_file->getMimeType();
        $check_mime_type = $file_model->checkMimeType($mime_type, "etc"); // 이미지 파일용 체크
        if($check_mime_type == false) {
            $result = false;
            $message = "업로드가 불가능합니다.";
        }
        ...
        $proc_result["message"] = $message;
        $proc_result["file_id"] = $file_id;
        $proc_result["file_path"] = "/attach/download/".$file_id;
        $proc_result["file_html"] = "<a href=\"/attach/download/".$file_id."\">".$file_name_org."</a>";
